...
5. **Test-first work (TDD: Red-Green-Refactor)** – Specify tests that must be written FIRST and fail before implementation (unit, contract, UI). Confirm Red-Green-Refactor cycle is followed. Manual validation steps must list owners and evidence.
6. **Conventional Commits** – All commits follow `<type>(<scope>): <description>` format (feat|fix|docs|test|refactor|chore|perf|ci). Each commit is atomic and includes tests.
